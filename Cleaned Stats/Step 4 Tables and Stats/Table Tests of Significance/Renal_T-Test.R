...
# Filter to Time Point == "0"
df_time0 <- df %>% filter(`Time Point` == "0")

# Filter to Vendor 0 or 1 only (adjust column name if different)
...
# Ensure other grouping variables are factors (adjust if names differ)
df_time0$`Hemorrhage Level` <- as.factor(df_time0$`Hemorrhage Level`)
df_time0$`Occlusion Group` <- as.factor(df_time0$`Occlusion Group`)  # Confirm this column exists in your data
